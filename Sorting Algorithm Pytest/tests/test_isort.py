...
df = pd.read_csv("C:/Users/harra/Desktop/Sorting Algorithm Pytest/DiamondValues.csv")
...
def test_sorted():
    #sorted == sorted
    start = time.time()
    assert i_sort.insertsort(sorted_datalist) == sorted_datalist
    end = time.time()
...
def test_unsorted():
    ...
    start = time.time()
    assert i_sort.insertsort(datalist) == sorted_datalist
    end = time.time()
    print("Execution Time: ", round(end - start, 3), "seconds")
...
def test_reversed():
    ...
    start = time.time()
    assert i_sort.insertsort(reverse_list) == sorted_datalist
    end = time.time()
    print("Execution Time: ", round(end - start, 3), "seconds")
